...
import { addNewTour } from '../../../api/lib/ToursAPI';

function CreateTourForm({ handlepopupClose, render, setRender, userId }) {
    ...
    const onSubmit = async (data) => {
        ...
        await addNewTour(data).then(() => { setRender(!render) })   //send data into database(depending on current UserId)
        ...
        <div className='popupform d-flex flex-column flex-nowrap'>
            <div className='formblock'>
                <div className='formtitle d-flex flex-row flex-nowrap pb-4 align-items-center p-4'>
                    <div className='border border-3 border-primary rounded text-center'><MdTour className='text-dark' /></div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className='d-flex flex-column flex-wrap text-center'>
                    ...
                    <p className=' p-0 text-danger'>{errors.tour?.message}</p>
                    <div className='formfooter d-flex flex-row flex-wrap mt-5'>
                        <div className='me-4'>
                            <button
                                className='w-55 btn text-light'
                                type='submit' id="btn" disabled={!tour}>Sukurti
                            </button>
                        </div>
                        <div className='me-4'>
                            <button
