...
import { getAllRestaurants } from '../../../api/lib/RestaurantAPI';
...
function MainAdminTable() {
  ...
  useEffect(() => {
    ...
    getAllRestaurants().then((res) => {
      ...
      setRestaurantId(...restaurantdata.map((data) => data._id));
    });
  }, [render, restaurantId]);
  useEffect(() => {
    let tempAll = [...restaurants];
    setAll(tempAll);
  }, [restaurants])
        ...
        <div className='mainadmincontent p-0 m-0'>
          <div className='header'>
            ...
            <div className='nav ps-5'>
              <Link to="/order" className='p-2 text-decoration-none text-light'><span className='text-center text-warning p-1 me-2 border-bottom border-warning'><MdOutlineCategory /></span>Užsakymai</Link>
              <Link to="/admin" className='p-2 text-decoration-none text-light'><span className='text-center text-warning p-1 me-2 border-bottom border-warning'><BsJournals /></span>Restoranai</Link>
            </div>
          </div>
